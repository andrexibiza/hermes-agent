...
export function resolvedConnectionId(
  ...
  const explicitConnectionId = String(descriptor.connectionId ?? '').trim()

  // The descriptor already crossed the authoritative pre-dial seam. Never
  // replace that proof with a weaker identity reconstructed after dialing.

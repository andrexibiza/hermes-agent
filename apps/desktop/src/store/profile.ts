...
export async function ensureGatewayAgent(connectionId: null | string, profile: string): Promise<void> {
  ...
  $gatewaySwapTarget.set(target)
  gatewaySwitch = (async () => {
    // Descriptor lookup and dial remain concurrent. Unlike the old boolean,
    // the receipt retains the exact source, profile, gateway object,
    // activation generation, and readiness through the publication boundary.
    const receipt = await activateGatewayAgentWithProof(
      connection,
      target,

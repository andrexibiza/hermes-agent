...
beforeEach(() => {
  ...
  activationProof.activateProfile.mockImplementation(
    async (profile: string, connectionPromise: Promise<HermesConnection | null>) => {
  ...
  activationProof.activateAgent.mockImplementation(
    async (connectionId: string, profile: string, connectionPromise: Promise<HermesConnection | null>) => {

...
const remoteConnection = (connectionId: string): HermesConnection =>
  ({
...
describe('desktop git facade', () => {
  ...
  it('rejects every remote Git mutation surface without current exact proof', () => {
    ...
    for (const mutate of mutations) {
      expect(mutate).toThrow('Remote Git mutation requires a current exact route activation')
    }
  ...
  it('keeps the remote branch-to-worktree flow under one exact mutation owner', async () => {

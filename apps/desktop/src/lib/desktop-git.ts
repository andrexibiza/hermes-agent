...
function desktopMutationApi<T>(path: string, body: Record<string, unknown>): Promise<T> {
  ...
  // consume the exact activation receipt at the side-effect boundary.
  if (!receipt || !connectionId) {
    throw new Error('Remote Git mutation requires a current exact route activation')
  }

  return desktopBridge().api<T>({
    body,
    connectionId,
    method: 'POST',
    path,
